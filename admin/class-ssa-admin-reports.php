...
class SSA_Admin_Reports {
	...
	public static function render() {
		...
		echo SSA_Admin_UI::card_open( __( 'Coupons by discount', 'splitshare-affiliates' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		if ( array_sum( $b ) > 0 ) {
			...
			), array( 'center' => number_format_i18n( array_sum( $b ) ), 'center_label' => __( 'coupons', 'splitshare-affiliates' ), 'title' => __( 'Live coupons by discount', 'splitshare-affiliates' ) ) );
			echo '<table class="ssa-table" style="margin-top:12px"><thead><tr><th>' . esc_html__( 'Discount / commission', 'splitshare-affiliates' ) . '</th><th class="num">' . esc_html__( 'Coupons', 'splitshare-affiliates' ) . '</th><th class="num">' . esc_html__( 'Orders', 'splitshare-affiliates' ) . '</th><th class="num">' . esc_html__( 'Revenue', 'splitshare-affiliates' ) . '</th></tr></thead><tbody>';
			foreach ( SSA_Partner_Coupons::discount_distribution( $r['from'], $r['to'] ) as $row ) {
				echo '<tr><td>' . SSA_Charts::split_bar( max( 0, $share - (float) $row->discount_pct ), (float) $row->discount_pct, $share, array( 'labels' => false ) ) . '<small class="ssa-muted">' . esc_html( wc_format_decimal( $row->discount_pct, 1 ) . '% / ' . wc_format_decimal( max( 0, $share - (float) $row->discount_pct ), 1 ) . '%' ) . '</small></td><td class="num">' . (int) $row->coupons . '</td><td class="num">' . (int) $row->orders . '</td><td class="num">' . wp_kses_post( wc_price( $row->revenue ) ) . '</td></tr>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			echo '</tbody></table>';
		} else {
			echo SSA_Admin_UI::empty_state( __( 'No coupons yet', 'splitshare-affiliates' ), __( 'Partners create coupons from My Account; their discount choices show up here.', 'splitshare-affiliates' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		echo SSA_Admin_UI::card_close(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		if ( $top ) {
			echo '<table class="ssa-table"><thead><tr><th>' . esc_html__( 'Partner', 'splitshare-affiliates' ) . '</th><th class="num">' . esc_html__( 'Live coupons', 'splitshare-affiliates' ) . '</th><th class="num">' . esc_html__( 'Orders', 'splitshare-affiliates' ) . '</th><th class="num">' . esc_html__( 'Revenue', 'splitshare-affiliates' ) . '</th><th class="num">' . esc_html__( 'Commission', 'splitshare-affiliates' ) . '</th></tr></thead><tbody>';
			foreach ( $top as $row ) {
				$p = SSA_Partners::get( $row->id );
				echo '<tr><td>' . ( $p ? SSA_Admin_Menu::partner_link( $p ) : '' ) . '</td><td class="num">' . (int) SSA_Partner_Coupons::count_active( $row->id ) . '</td><td class="num">' . (int) $row->orders . '</td><td class="num">' . wp_kses_post( wc_price( $row->revenue ) ) . '</td><td class="num"><strong>' . wp_kses_post( wc_price( $row->commission ) ) . '</strong></td></tr>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			echo '</tbody></table>';
		} else {
		...
		if ( $an['rows'] ) {
			...
			foreach ( $an['rows'] as $row ) {
				echo '<tr><td><code>' . esc_html( $row->code ) . '</code></td><td class="num">' . (int) $row->orders . '</td><td class="num">' . esc_html( $row->new_rate ) . '%</td><td>' . ( $row->flag ? '<span class="ssa-pill ssa-pill--bad"><i></i>' . esc_html__( 'Suspicious', 'splitshare-affiliates' ) . '</span> <a class="ssa-link" href="' . esc_url( SSA_Admin_Menu::url( 'partners', array( 'action' => 'edit', 'id' => $row->partner_id ) ) ) . '">' . esc_html__( 'Manage coupons', 'splitshare-affiliates' ) . '</a>' : '' ) . '</td></tr>';
			}
			echo '</tbody></table>';
		} else {
